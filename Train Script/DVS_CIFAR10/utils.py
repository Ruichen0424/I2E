...
def get_dataset(args):
    if args.dataset.lower() == 'cifar10':
        ...
        return Train_data, Test_data
    elif args.dataset.lower() == 'dvscifar10':

        from spikingjelly.datasets.cifar10_dvs import CIFAR10DVS
        from spikingjelly.datasets import split_to_train_test_set

        Dts_cache = args.dataset_path + 'dts_cache/'
        train_set_pth = os.path.join(Dts_cache, f'train_set_{8}.pt')
        test_set_pth = os.path.join(Dts_cache, f'test_set_{8}.pt')
        if os.path.exists(train_set_pth) and os.path.exists(test_set_pth):
            ...
            origin_set = CIFAR10DVS(root=args.dataset_path, data_type='frame', frames_number=8, split_by='number')
